...
use crate::algorithms::{generate_multi_prime_key, generate_multi_prime_key_with_fixed_primes};
...
impl RSAPrivateKey {
    ...
    /// Generate a new RSA key pair of the given bit size using the passed in `rng`.
    pub fn new_with_primes<R: Rng>(rng: &mut R, bit_size: usize, start_primes: Vec<BigUint>) -> Result<RSAPrivateKey> {
        generate_multi_prime_key_with_fixed_primes(rng, 2, bit_size, start_primes)
    }
...
mod tests {
    ...
    #[test]
    fn test_fixed_primes_keys() {
        // make sure not infinite loops are hit here.
        let mut rng = thread_rng();

        let first_key:RSAPrivateKey = generate_multi_prime_key(&mut rng, 2, 1024).unwrap();

        let fixed_key:RSAPrivateKey = generate_multi_prime_key_with_fixed_primes(&mut rng, 2, 1024,  first_key.primes().to_vec()).unwrap();

        test_key_basics(&fixed_key);
